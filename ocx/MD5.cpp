 ...
 */
 void MD5::MD5Transform (UINT4 state[],unsigned char block[])
//UINT4 state[4];static
...
BOOL MD5::MD5Hash(CString m_EditIn,TCHAR szBuffer[]) ///还有一些问题
{
 MD5_CTX context;
 long dtLength;
 int i;
...
// TCHAR szBuffer[MAXINPUTLEN]={0};sizeof(szBuffer)
 memset(szBuffer,0,MAXINPUTLEN);

 dtLength = strlen(m_EditIn);/////////  
 strcpy(szName,m_EditIn);
// CString a;
// a.Format("%d",dtLength);
// AfxMessageBox(a);
 //++++++++++++++++++++++++++++strlen(szName)
 ...
 MD5Init(&context);
 MD5Update(&context, (unsigned char *)szName, dtLength);//括号里自己加的
 MD5Final((unsigned char *)szHash, &context);//括号里自己加的
