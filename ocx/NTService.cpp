...
BOOL CNTService::IsInstalled()
{
    ...
    SC_HANDLE hSCM = ::OpenSCManager(NULL, // local machine
		...
		SC_MANAGER_CONNECT); // full access
    if(hSCM) 
	{
...
BOOL CNTService::Install(CString& strFilePath )
{
	// Open the Service Control Manager
	SC_HANDLE hSCM = ::OpenSCManager(NULL, // local machine
		NULL, // ServicesActive database
	...
	::RegSetValueEx(hKey,
		...
		0,
		REG_EXPAND_SZ, 
		(CONST BYTE*)strFilePath.GetBuffer(0),
		strlen(strFilePath.GetBuffer(0)) + 1);     
	// Set the supported types flags.
	DWORD dwData = EVENTLOG_ERROR_TYPE | EVENTLOG_WARNING_TYPE | EVENTLOG_INFORMATION_TYPE;
	::RegSetValueEx(hKey,
		"TypesSupported",
...
void CNTService::OnDebug()
{
	DebugMsg("CNTService::OnDebug()");
}

void CNTService::OnRelease()
{
	DebugMsg("CNTService::OnRelease()");
}
...
BOOL CNTService::CmdStartSrv()
{
	...
	SC_HANDLE schService = OpenService(schSCManager,m_szServiceName, SERVICE_START|SERVICE_QUERY_STATUS);   
	if(schService ==  NULL)   
	{ 
...
BOOL CNTService::CmdStopSrv()
{
	...
	SC_HANDLE schService = OpenService(schSCManager,m_szServiceName,SERVICE_STOP|SERVICE_QUERY_STATUS);   
	if(schService ==  NULL)   
	{ 
		CloseServiceHandle(schSCManager);   
		return FALSE;
